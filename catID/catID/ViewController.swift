...
//  catID
//
//  Created by Chris Mlinac on 6/20/20.
//  Copyright © 2020 CML. All rights reserved.
//
...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    // create a label to hold the Cat name and confidence
    let label: UILabel = {
        let label = UILabel()
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        guard let model = try? VNCoreMLModel(for: cat().model) else { return }
